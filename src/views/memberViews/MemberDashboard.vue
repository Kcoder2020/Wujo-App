      ...
      <div class="hero-section">
        <div class="hero-header">
          ...
          <div
            ...
            @click="goToLotteryNotifications"
          >
            <ion-icon :icon="notificationsOutline" class="hero-icon"></ion-icon>
            <span v-if="unviewedLotteryCount > 0" class="notification-badge">
              {{ unviewedLotteryCount > 99 ? "99+" : unviewedLotteryCount }}
            </span>
          </div>
        </div>
...
const recentActivities = computed(() => {
  const activities = dashboardData.value?.recentActivities || [];
  // Transform API data to view format
  return activities.map((activity: any, index: number) => ({
    id: `activity-${index}`,
    type: "payment",
    title: `Round ${activity.roundNumber} Payment`,
    description: `${activity.iqubName} - ${formatCurrency(activity.amount)} (${
      activity.status
    })`,
...
const goToLotteryNotifications = () => {
  router.push("/member/lottery-notifications");
};
